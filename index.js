...
async function runTask(
  ...
  const taskArns = runTaskResponse.tasks.map((task) => {
    ...
    core.info(
      `Task started. Watch this task's progress in the Amazon ECS console: https://console.aws.amazon.com/ecs/home?region=${aws.config.region}#/clusters/${clusterName}/tasks/${taskIdentity}/details`
    );
    return task.taskArn;
  });
...
async function run() {
  try {
    ...
    const taskCount =
      parseInt(core.getInput("count", { required: false })) || 1;
